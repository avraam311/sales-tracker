...
import (
	...
	handlerAn "github.com/avraam311/sales-tracker/internal/api/handlers/analytics"
	handlerSales "github.com/avraam311/sales-tracker/internal/api/handlers/sales"
	"github.com/avraam311/sales-tracker/internal/api/server"
	repositoryAn "github.com/avraam311/sales-tracker/internal/repository/analytics"
	repositorySales "github.com/avraam311/sales-tracker/internal/repository/sales"
	serviceAn "github.com/avraam311/sales-tracker/internal/service/analytics"
	serviceSales "github.com/avraam311/sales-tracker/internal/service/sales"
...
func main() {
	...
	repoSales := repositorySales.NewRepository(db)
	srvcSales := serviceSales.NewService(repoSales)
	handSales := handlerSales.NewHandler(srvcSales, val)
	repoAn := repositoryAn.NewRepository(db)
	srvcAn := serviceAn.NewService(repoAn)
	handAn := handlerAn.NewHandler(srvcAn, val)

	router := server.NewRouter(cfg.GetString("server.gin_mode"), handSales, handAn)
